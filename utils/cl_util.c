 ...
 * $Id: cl_util.c,v 1.1.1.3 2021/07/29 00:00:00 seiji Exp seiji $
 */
...
// prototypes
static cl_device_id  clFindTargetDevice_(const char *, cl_device_type, cl_uint     );
static cl_program    clCompileKernel_   (cl_obj_t   *, const char   *, const char *);
...
{				// initialize OpenCL.
    ...
    // find the target OpenCL device.
    obj->device  = clFindTargetDevice_(platform_name, device_type, device_num);
    ...
    // OpenCL kernel JIT compilation
    obj->program = clCompileKernel_(obj, kernel, build_options);
...
//----------------------------------------------------------------------
static
cl_device_id clFindTargetDevice_
	(const char *platform_name, cl_device_type device_type, cl_uint device_num)
{				// find the target OpenCL device.
    cl_platform_id platform[N_TBL];
    cl_device_id   dev_id  [N_TBL], device;
    ...
    for (int i = 0; i < num_platforms; i++) {	// search all platforms.
	char pname[256];
	status = clGetPlatformInfo(platform[i], CL_PLATFORM_NAME, sizeof(pname), pname, NULL);
	cl_check_status(status);
	if (platform_name != NULL)	// platform_name is given, but current platform does not match.
	    if (strcasecmp(platform_name, pname) != 0)
		continue;
...
#ifdef CL_UTIL_DEBUG
	    char dname[256];
	    status = clGetDeviceInfo(dev_id[device_num], CL_DEVICE_NAME, sizeof(dname), dname, NULL);
	    cl_check_status(status);
	    printf("OpenCL\n");
	    printf("  Platform: %s\n", pname);
	    printf("  Device  : %s\n", dname);
#endif
	    device = dev_id[device_num];
	    break;
	} else {
...
//----------------------------------------------------------------------
static
cl_program clCompileKernel_
	(cl_obj_t *obj, const char *kernel, const char *build_options)
{				// OpenCL kernel JIT compilation
    size_t  kernel_size;
    char   *kernel_src ;
    cl_device_id device;
	...
	program = clCreateProgramWithSource
			(context, 1, (const char **) &kernel_src, &kernel_size, &status);
	cl_check_status(status);
    } else {	// clDetectKernelType_(kernel) == BINARY
...
{				// detect file type of OpenCL kernel.
	...
	char  *suffix = type_table[i].suffix ;
	size_t suffix_length = strlen(suffix);
	if (fname_length > suffix_length &&
	    strcasecmp(&fname[fname_length - suffix_length], suffix) == 0) {
